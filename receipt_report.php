...
// Fetch receipt (income) data from the database with the corrected table name 'Sell'
$receipts = [];
$sql = "SELECT 
            s.s_id,
            s.date,
...
<div class="receipt-container">
    <div class="container no-print" style="text-align:ສຳດະ;margin-top:20px;">
        <button class="Print-btn" onclick="window.print();">ພິມລາຍງານ</button>
    </div>
    
    <h2 class="receipt-title">ໃບລາຍງານລາຍຮັບ<br>Income Report Receipt</h2>
    ...
    <table class="receipt-table">
        ...
        <tbody>
            ...
            ?>
                <tr>
                    <td><?php echo htmlspecialchars($receipt['s_id']); ?></td>
                    <td><?php echo date('d/m/Y', strtotime($receipt['date'])); ?></td>
                    <td class="text-left"><?php echo htmlspecialchars($receipt['c_name'] ?? 'N/A'); ?></td>
                    <td class="text-left"><?php echo htmlspecialchars($receipt['emp_name'] ?? 'N/A'); ?></td>
                    <td><?php echo htmlspecialchars($receipt['total_items']); ?></td>
                    <td><?php echo laoNumberFormat($receipt['total_amount']); ?></td>
                </tr>
            <?php 
                endforeach;
            ?>
                <tr>
                    <td colspan="5" style="text-align:right;font-weight:bold;">ລາຍຮັບລວມທັງໝົດ</td>
            ...
            ?>
                <tr>
                    <td colspan="6">ບໍ່ມີຂໍ້ມູນລາຍຮັບ.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
